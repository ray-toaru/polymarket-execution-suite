...
class V028DocsAlignmentTests(unittest.TestCase):
    def test_execution_engine_roadmap_reflects_v028_current_state(self):
        text = read("polymarket-execution-engine/docs/ROADMAP.md")
        self.assertIn("v0.28 development status", text)
        self.assertIn("Already landed for v0.28", text)
        self.assertIn("Remaining before v0.28 release", text)
    ...
    def test_adapter_docs_reflect_executor_adapter_v028_state(self):
