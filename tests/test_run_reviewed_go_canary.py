...
PROFILE_PK = "PMX_PROFILE_ACCT_B_" + PK
...
class RunReviewedGoCanaryTests(unittest.TestCase):
    ...
    def test_build_invocation_preflight_uses_package_files_without_live_submit_gate(self):
        with tempfile.TemporaryDirectory() as tmp_name:
            ...
            plan = self.module.build_invocation(package_dir=package, env_file=env_file, mode="preflight", daily_used_notional_usd="0", idempotency_key=None, execution_id=None, plan_hash=None, report_file=None, approval_consumed_marker=None)
        self.assertEqual(plan["mode"], "preflight")
        self.assertIn("--preflight-only", plan["command"])
        self.assertNotIn("PMX_ALLOW_LIVE_SUBMIT", plan["required_gate_env_vars"])

    def test_build_invocation_armed_defaults_collision_resistant_marker(self):
        with tempfile.TemporaryDirectory() as tmp_name:
            package, env_file = self.package_fixture(Path(tmp_name))
            plan = self.module.build_invocation(package_dir=package, env_file=env_file, mode="armed", daily_used_notional_usd="0", idempotency_key=None, execution_id=None, plan_hash=None, report_file=None, approval_consumed_marker=None)
        self.assertIn("approval-consumed-dddddddddddd-exec-dddddddddddd", plan["approval_consumed_marker"])
    ...
    def test_build_invocation_rejects_consumed_package(self):
        with tempfile.TemporaryDirectory() as tmp_name:
            ...
            with self.assertRaisesRegex(SystemExit, "already consumed"):
                self.module.build_invocation(package_dir=package, env_file=env_file, mode="preflight", daily_used_notional_usd="0", idempotency_key=None, execution_id=None, plan_hash=None, report_file=None, approval_consumed_marker=None)

    def test_validate_approval_rejects_expired_approval(self):
        with tempfile.TemporaryDirectory() as tmp_name:
            package, _ = self.package_fixture(Path(tmp_name))
            approval_file = package / "approval.json"
            ...
            with self.assertRaisesRegex(SystemExit, "approval is expired"):
                self.module.validate_approval(approval_file)

    def test_build_subprocess_env_strips_profile_inventory_and_ambient_secrets(self):
        with tempfile.TemporaryDirectory() as tmp_name:
        ...
        self.assertEqual(env[API_SECRET], "dummy_secret")
        self.assertNotIn(PROFILE_PK, env)
        self.assertEqual(env["PMX_KILL_SWITCH_OPEN"], "1")

    def test_main_run_passes_isolated_env_to_subprocess(self):
        with tempfile.TemporaryDirectory() as tmp_name:
            package, env_file = self.package_fixture(Path(tmp_name))
            original = dict(os.environ)
            original_parse_args = self.module.parse_args
            try:
                os.environ.clear(); os.environ.update({key: "1" for key in self.module.PREFLIGHT_GATE_ENV_VARS}); os.environ["PATH"] = "/bin"; os.environ[PROFILE_PK] = "ambient"
                self.module.parse_args = lambda: self.module.argparse.Namespace(package_dir=package, env_file=env_file, mode="preflight", daily_used_notional_usd="0", idempotency_key=None, execution_id=None, plan_hash=None, report_file=None, approval_consumed_marker=None, run=True)
                ...
                os.environ.clear(); os.environ.update(original)
        self.assertEqual(passed_env["PATH"], "/bin")
        self.assertNotIn(PROFILE_PK, passed_env)
